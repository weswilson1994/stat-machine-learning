...
Though we had variables on the number of tickets and warnings issued, it did not make sense from a practical standpoint, as these variables are key indicators or perfectly linear with stop outcome. KNN performed best due to the non-parametric nature. LDA and QDA performed the worst. Interestingly, QDA did the absolute worst because the QDA function would not allow for any categorical variable.

Ultimately, the non-binary classification approach might not be the best. It likely would have been better. For some reason, the lasso regression model had a 100% model accuracy.

-   When we include all of the variables in the model, there is too much noise. That is the variance within the x values is too high. As a result, our prediction accuracy actually decreases. So, we go back and perform lasso to regularize the model and select only relevant variables. Unfortunately, the lasso regression keeps all variables.
